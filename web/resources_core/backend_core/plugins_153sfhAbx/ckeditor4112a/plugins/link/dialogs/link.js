...
console.log(plugin);
	...
	var parseLink = function( editor, element ) {
			...
			if ( element ) {
                ...
                if ( !retval.qtlink) {
                    var qtlink = element.getAttribute('data-qtlink');
                    retval.qtlink = qtlink;
                }
			...
			//retval.anchors = plugin.getEditorAnchors( editor );
    ...
    var urls_loaded = false;
    var isloading = false;

    function loadPredefined(linkelem) {
        var element = $('#' + this.getInputElement().$.id);
        var value;

        if (linkelem.qtlink) {
            value = linkelem.qtlink;
        }
                    ...
                    if (!urls_loaded) {
                        ...
                        for (var i in data) {
                            ...
                            for (var option in data[i]) {
                                ...
                                optgroup.append('<option value="' + option + '" ' + isSel + '>' + data[i][option] + '</option>');
                            }
	...
	return {
                ...
                children: [
                    {
                        ...
                        onChange: function() {
                            ...
                            urlfield.setValue(this.getValue());
                        },
                        setup: loadPredefined,
                        onLoad: loadPredefined,
                        commit: function( data ) {
                            if ( !data.qtlink )
                                data.qtlink = "";

                            data.qtlink = this.getValue();

                            if ( !data.url )
                                data.url = {};
                            data.url.url = data.qtlink;
                        }
                    }
                ]
            }/*,
		...
		onOk: function() {
            ...
            if (data.qtlink != "") {
                attributes['data-qtlink'] = data.qtlink;
            } else {
                delete attributes['data-qtlink'];
                removeAttributes.push('data-qtlink');
            }
